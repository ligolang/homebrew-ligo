...
current_release_name="$(grep -Po 'tag: "\K.*?(?=")' Formula/ligo.rb)"
...
if [ "$current_release_name" = "$latest_release_name" ]; then
    ...
    sed -i "s|tag: \".*\", revision: \".*\"|tag: \"${latest_release_name}\", revision: \"${latest_release_revision}\"|g" Formula/ligo.rb
fi

latest_next_commit_hash="$(curl -X GET "https://gitlab.com/api/v4/projects/12294987/repository/commits" | jq --raw-output '.[0].id')"
current_next_commit_hash="$(grep -Po 'revision: "\K.*?(?=")' Formula/ligo_next.rb)"
...
if [ "$current_next_commit_hash" = "$latest_next_commit_hash" ]; then
    ...
    sed -i "s|${current_next_commit_hash}|${latest_next_commit_hash}|g" Formula/ligo_next.rb
fi